...
import 'package:clean_archeticture_movies/core/utiles/constance.dart';
...
import '../../domen/entities/genres.dart';


import '../../../core/service/service_local.dart';
import '../controller/bloc_details/movies_details_bloc.dart';
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => sl<MoviesDetailsBloc>()
        ..add(GetMoviesDeatilsEvent(id))
        ..add(GetMoviesRecomendation(id)),
      child: Scaffold(
      ...
      builder: (context, state) {
        switch (state.moviesDetailsState) {
          ...
          case RequestState.erorr:
            return Center(child: Text(state.moviesDetailsmeesage));
        }
      },
    );
  }
      ...
      builder: (context, state) {
        return SliverGrid(
          delegate: SliverChildBuilderDelegate(
            (context, index) {
              final recommendation = state.moviesRecomendation![index];
              return FadeInUp(
                from: 20,
                duration: const Duration(milliseconds: 500),
                child: ClipRRect(
            ...
            childCount: state.moviesRecomendation!.length,
          ),
